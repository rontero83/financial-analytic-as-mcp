...
| `FSMC_FREE_SPACE_MB`                              | `100`            | Positive integer. Disk-space threshold in megabytes; `create_task` refuses with a `DISK_FULL` error when free space on the tasks volume is below this.                        | Non-integer, zero, or negative -> `sys.exit(5)` with an actionable stderr message.                                              |
...
| `5`  | Invalid `FSMC_FREE_SPACE_MB` (non-int, zero, or negative). Introduced by `sys.exit(5)` in plan 03-02. |
